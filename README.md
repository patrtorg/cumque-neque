# @patrtorg/cumque-neque

Parse JSON without risk of losing numeric information.

```js
import { parse, stringify } from '@patrtorg/cumque-neque'
...
**How does it work?** The library works exactly the same as the native `JSON.parse` and `JSON.stringify`. The difference is that `@patrtorg/cumque-neque` preserves information of big numbers. `@patrtorg/cumque-neque` parses numeric values not as a regular number but as a `LosslessNumber`, a lightweight class which stores the numeric value as a string. One can perform regular operations with a `LosslessNumber`, and it will throw an error when this would result in losing information.

**When to use?** If you have to deal with JSON data that contains `long` values for example, coming from an application like C++, Java, or C#. The trade-off is that `@patrtorg/cumque-neque` is slower than the native `JSON.parse` and `JSON.stringify` functions, so be careful when performance is a bottleneck for you.
...
Install via [npm](https://www.npmjs.com/package/@patrtorg/cumque-neque):

```
npm install @patrtorg/cumque-neque
```
...
```js
import { parse } from '@patrtorg/cumque-neque'
...
```js
import { parse, isInteger } from '@patrtorg/cumque-neque'
...
```js
import { parse, isSafeNumber } from '@patrtorg/cumque-neque'
...
- `reviveDate(key, value)`
  ...
  ```js
  import { parse, reviveDate } from '@patrtorg/cumque-neque'
...
(Spoiler: `@patrtorg/cumque-neque` is much slower than native)
